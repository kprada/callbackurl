...
docManager.prototype.saveFileData = function (fileName, userid, username, userAddress,carpeta) {
   /*  if (!userAddress) {
        userAddress = this.curUserHostAddress();  // get current user host address
    } */
    // get full creation date of the document
    const date_create = fileSystem.statSync(this.storagePath(fileName, userAddress,carpeta)).mtime;
    ...
    const file_info = this.historyPath(fileName, userAddress, true,carpeta);  // get file history information
...
docManager.prototype.storageRootPath = function (userAddress) {
    return path.join(storageConfigFolder,userAddress );  // get the path to the directory for the host address
}

// get the storage path of the given file
docManager.prototype.storagePath = function (fileName, userAddress,carpeta) {
    ...
    if (carpeta){
        let rutaCarpeta = path.join(directory, carpeta); //files/localhost/controldocument
        let root = path.join(userAddress, carpeta);
        this.createDirectory(rutaCarpeta);
        directory= this.storageRootPath(root);
    }
    return path.join(directory, fileName);  // put the given file to this directory
};
// get the correct file name if such a name already exists
docManager.prototype.getCorrectName = function (fileName, userAddress,carpeta) {
    ...
    while (this.existsSync(this.storagePath(name, userAddress,carpeta))) {  // if the file with such a name already exists in this directory
...
// get file data
docManager.prototype.getFileData = function (fileName, userAddress,carpeta) {
    const history = path.join(this.historyPath(fileName, userAddress, true,carpeta), fileName + ".txt");  // get the path to the file with file information
...
docManager.prototype.forcesavePath = function (fileName, userAddress, create,carpeta) {
    let root =userAddress;
     if (carpeta){
        root = path.join(userAddress, carpeta);
    }
    let directory = this.storageRootPath(root);
...
docManager.prototype.historyPath = function (fileName, userAddress, create,carpeta) {
    let root =userAddress;
    if(carpeta){
        root=path.join(userAddress,carpeta);
    }
...
docManager.prototype.versionPath = function (fileName, userAddress, version,carpeta) {

    const historyPath = this.historyPath(fileName, userAddress, true,carpeta);  // get the path to the history of a given file or create it if it doesn't exist
    return path.join(historyPath, "" + version);
};

// get the path to the previous file version
docManager.prototype.prevFilePath = function (fileName, userAddress, version,carpeta) {
    return path.join(this.versionPath(fileName, userAddress, version,carpeta), "prev" + fileUtility.getFileExtension(fileName));
};

// get the path to the file with document versions differences
docManager.prototype.diffPath = function (fileName, userAddress, version,carpeta) {
    return path.join(this.versionPath(fileName, userAddress, version,carpeta), "diff.zip");
};

// get the path to the file with document changes
docManager.prototype.changesPath = function (fileName, userAddress, version,carpeta) {
    return path.join(this.versionPath(fileName, userAddress, version,carpeta), "changes.txt");
};

// get the path to the file with key value in it
docManager.prototype.keyPath = function (fileName, userAddress, version,carpeta) {
    return path.join(this.versionPath(fileName, userAddress, version,carpeta), "key.txt");
};

// get the path to the file with the user information
docManager.prototype.changesUser = function (fileName, userAddress, version,carpeta) {
    return path.join(this.versionPath(fileName, userAddress, version,carpeta), "user.txt");
};
...
// get document key
docManager.prototype.getKey = function (fileName, userAddress,carpeta) {
    ...
    let historyPath = this.historyPath(fileName, userAddress,carpeta);  // get the path to the file history
    ...
    let storagePath = this.storagePath(fileName, userAddress,carpeta);  // get the storage path to the given file
