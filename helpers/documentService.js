...
//actualizacion de key cuando el estado es 2 o 3
documentService.updateKey = async(oldkey,origin,conexion)=>{
let response;
    try {
          response = await axios.get( origin+conexion+"/ControlDocument/actualizarKey?oldkey="+oldkey);
...
documentService.infoNewEtapa = async(origin,conexion,codarchivo)=>{
let response;
    try {
          response = await axios.get( origin+conexion+"/ControlDocument/verificarSiguienteEtapa?codarchivo="+codarchivo);
         if (response.data.success) {
...
documentService.createNewEtapa = async(origin,conexion,body)=>{
let response;
    try {
          response = await axios.post( origin+conexion+"/ControlDocument/guardarArchivoSeccion",
...
documentService.generateNewKey =function(oldkey){
 let newKey =uuid.v1().toString().replace(/-/g,"");
 let maxKeyLength = 128;  // the max key length is 128
 return newKey.substring(0, Math.min(newKey.length, maxKeyLength));  // the resulting key is of the max key length or less
} 
